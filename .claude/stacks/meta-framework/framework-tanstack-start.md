...
**DO NOT use `createServerFn` in route files that import Prisma, Better Auth, or other server-only packages.**
...
**Pattern - API Route (`src/routes/api/settings.ts`):**
```typescript
import { createFileRoute } from "@tanstack/react-router";
...
**Pattern - Client Route (`src/routes/_app/settings.tsx`):**
```typescript
import { createFileRoute } from "@tanstack/react-router";
...
1. **API routes** at `src/routes/api/*.ts` are purely server-side and safe for Prisma/auth
2. **Route files** should only import client-safe modules
3. Create shared type files (`*.shared.ts`) for types/constants used by both client and API
4. Use `Response.json()` (native Web API) not `json()` import from tanstack/react-router
5. Client routes use `fetch()` to call API routes with `credentials: "include"` for auth cookies
...
|--------|-------------|------|
| Start | Full-stack React framework (this guide) | [tanstack.com/start](https://tanstack.com/start) |
| Router | Type-safe file-based routing | [tanstack.com/router](https://tanstack.com/router) |
| Query | Async state management and caching | [tanstack.com/query](https://tanstack.com/query) |
| Table | Headless UI for building tables and datagrids | [tanstack.com/table](https://tanstack.com/table) |
| Form | Type-safe form state management | [tanstack.com/form](https://tanstack.com/form) |
| Virtual | Virtualize large lists for performance | [tanstack.com/virtual](https://tanstack.com/virtual) |
| DB | Reactive client store with sync engines (alpha) | [tanstack.com/db](https://tanstack.com/db) |
| AI | AI utilities with Standard Schema support (alpha) | [tanstack.com/ai](https://tanstack.com/ai) |
